...
var _clientsViewJs = require("./views/clientsView.js");
var _clientsViewJsDefault = parcelHelpers.interopDefault(_clientsViewJs);
...
const controlClients = async function() {
    try {
        (0, _clientsViewJsDefault.default).renderSpinner();
        // 1. загрузка клиентов + имитация загрузки
        setTimeout(async ()=>{
            await _modelJs.loadClients();
            if (_modelJs.state.clients.length === 0) (0, _clientsViewJsDefault.default).renderEmptyTable();
            else (0, _clientsViewJsDefault.default).render(_modelJs.state.clients);
        }, 1000);
...
const controlModal = async function(id, option = true) {
    try {
        ...
        if (!id) return (0, _modalViewJsDefault.default).render();
        // 1. загрузка данных клиента из строки (с сервера)
        await _modelJs.getClient(id);
        // 2. отрисовка модального окна
        (0, _modalViewJsDefault.default).render(_modelJs.state.client, option);
    /**
...
const controlSubmitClient = async function(newData) {
    try {
        await _modelJs.uploadClient(newData);
        await _modelJs.loadClients();
        (0, _clientsViewJsDefault.default).renderEmptyTable();
        (0, _clientsViewJsDefault.default).render(_modelJs.state.clients);
...
const controlSortingClients = async function(sortType) {
    try {
        await _modelJs.loadClients(sortType);
        (0, _clientsViewJsDefault.default).render(_modelJs.state.sortClients);
    } catch  {
        console.log(err);
    }
};
const init = function() {
    (0, _clientsViewJsDefault.default).addHandlerRender(controlClients);
    (0, _clientsViewJsDefault.default).addHandlerShowModal(controlModal);
    (0, _modalViewJsDefault.default).addHandlerSubmit(controlSubmitClient);
    (0, _sortViewJsDefault.default).addHandlerSorting(controlSortingClients);
};
init();

},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3","core-js/modules/web.immediate.js":"49tUX","./model.js":"Py0LO","./views/clientsView.js":"lXL8Y","./views/modalView.js":"lYcLT","./views/sortView.js":"lozop"}],"gkKU3":[function(require,module,exports) {
...
parcelHelpers.export(exports, "loadClients", ()=>loadClients);
parcelHelpers.export(exports, "getClient", ()=>getClient);
parcelHelpers.export(exports, "uploadClient", ()=>uploadClient);
parcelHelpers.export(exports, "loadClientsToServer", ()=>loadClientsToServer);
parcelHelpers.export(exports, "deleteClient", ()=>deleteClient);
...
const loadClients = async function(sortType) {
    try {
        //получаем массив с сервера
        const data = await (0, _helpers.getJSON)((0, _config.SERVER_URL));
        //массив для отрисовки
        state.clients = data.map((clientObj)=>{
            return (0, _helpers.prepareClientToRender)(clientObj);
        });
        if (!sortType) return;
        ...
        if (sortType === "sortByName") {
            state.sortClients = state.clients.sort((a, b)=>b.surname + b.name + b.lastname - (a.surname + a.name + a.lastname));
            console.log(state.sortClients, sortType);
        }
        if (sortType === "sortByCreate") {
            state.sortClients = state.clients.sort((a, b)=>(0, _helpers.prepareToSortByDate)(a.createdAt) - (0, _helpers.prepareToSortByDate)(b.createdAt));
            console.log(state.sortClients, sortType);
        }
        if (sortType === "sortByEdit") {
            state.sortClients = state.clients.sort((a, b)=>(0, _helpers.prepareToSortByDate)(a.updatedAt) - (0, _helpers.prepareToSortByDate)(b.updatedAt));
            console.log(state.sortClients, sortType);
        }
    } catch (err) {
        console.error(err);
...
const getClient = async function(id) {
    try {
        const data = await (0, _helpers.getJSON)(`${(0, _config.SERVER_URL)}${id}`);
        state.client = data;
    } catch (err) {
        throw err;
    }
};
const uploadClient = async function(newData) {
    try {
        await (0, _helpers.sendJSON)(`${(0, _config.SERVER_URL)}${newData.id || ""}`, newData);
    } catch (err) {
        throw err;
    }
};
const loadClientsToServer = function(newData) {};
const deleteClient = async function() {};

},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3","./config":"4Wc5b","./helpers":"6s1be"}],"4Wc5b":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
parcelHelpers.export(exports, "prepareToSortByDate", ()=>prepareToSortByDate);
parcelHelpers.export(exports, "prepareClientToRender", ()=>prepareClientToRender);
var _config = require("./config");
const timeout = function(sec) {
    return new Promise(function(_, reject) {
        setTimeout(function() {
            reject(new Error(`\u{41D}\u{435}\u{442} \u{43E}\u{442}\u{432}\u{435}\u{442}\u{430} \u{441} \u{441}\u{435}\u{440}\u{432}\u{435}\u{440}\u{430} \u{432} \u{442}\u{435}\u{447}\u{435}\u{43D}\u{438}\u{438} ${sec} \u{441}\u{435}\u{43A}\u{443}\u{43D}\u{434} (\u{43D}\u{438}\u{437}\u{43A}\u{430}\u{44F} \u{441}\u{43A}\u{43E}\u{440}\u{43E}\u{441}\u{442}\u{44C} \u{438}\u{43D}\u{442}\u{435}\u{440}\u{43D}\u{435}\u{442}\u{430})`));
        }, sec * 1000);
    });
};
...
const sendJSON = async function(url, uploadData) {
    try {
        let res;
        if (uploadData.flag === "new") res = await fetch(url, {
            method: "POST",
            headers: {
        ...
        if (uploadData.flag === "delete") res = await fetch(url, {
            method: "DELETE"
        });
        const data = await res.json();
        if (!res.ok) throw new Error(`${data.message} (${res.status})`);
...
},{"./config":"4Wc5b","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"lXL8Y":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
      ...
      <span class="tooltip contact-item__tooltip">
        <button class="tooltip-toggle btn-reset" type="button">
          <svg class="contact-icon">
            <use href="${0, _iconsSvgDefault.default}#${contact.type}"></use>
          </svg>
        </button>
    ...
    _generateMarkup() {
        ...
        ${this._data.map((clientObj)=>{
            ...
            <tr class="table__row row--body" data-id="${clientObj.id}">
                ...
                ${clientObj.createdAt.date} <span class="body-text--time">${clientObj.createdAt.time}</span>
                </td>
                <td class="table__text body-text column-4">
                ${clientObj.updatedAt.date} <span class="body-text--time">${clientObj.updatedAt.time}</span>
                </td>
                ...
                <button class="table__btn btn-reset btn--edit ">
                    \u{418}\u{437}\u{43C}\u{435}\u{43D}\u{438}\u{442}\u{44C}
                </button>
...
exports.default = new ClientsView();

},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3","url:../../images/icons.svg":"4Uvst","./view":"7MbWE"}],"4Uvst":[function(require,module,exports) {
module.exports = require("d004fb8950a5edfd").getBundleURL("f3nP6") + "icons.5997c2f9.svg" + "?" + Date.now();

},{"d004fb8950a5edfd":"lgJ39"}],"lgJ39":[function(require,module,exports) {
"use strict";
var bundleURL = {};
...
class View {
    ...
    render(data, option) {
        ...
        if (option && this._getTypeOf(this._data) === "Object" || !option && !this._data) {
            this._parentEl.setAttribute("data-flag", `${option ? "edit" : "new"}`);
            this._parentEl.setAttribute("data-id", `${option ? `${this._data.id}` : ""}`);
    ...
    <div class="table__spinner spinner">
      <div class="spinner">
        <svg class="spinner-ico">
          <use href="${(0, _iconsSvgDefault.default)}#spinner"></use>
        </svg>
      </div>
    </div>
    `;
    ...
    <div class="table__spinner spinner">
      <div class="spinner">
        <p>\u{421}\u{43F}\u{438}\u{441}\u{43E}\u{43A} \u{43F}\u{443}\u{441}\u{442}, \u{434}\u{43E}\u{431}\u{430}\u{432}\u{442}\u{435} \u{43D}\u{43E}\u{432}\u{43E}\u{433}\u{43E} \u{43A}\u{43B}\u{438}\u{435}\u{43D}\u{442}\u{430}</p>
      </div>
    </div>
    `;
        this._clear();
...
},{"url:../../images/icons.svg":"4Uvst","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"lYcLT":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
var _view = require("./view");
var _viewDefault = parcelHelpers.interopDefault(_view);
...
class ModalView extends (0, _viewDefault.default) {
    ...
    <div class="form__full-name" data-id='edit'>
        <h2 class="modal__title text-reset">${this?._data ? "\u0418\u0437\u043C\u0435\u043D\u0438\u0442\u044C \u0434\u0430\u043D\u043D\u044B\u0435" : "\u0414\u043E\u0431\u0430\u0432\u0438\u0442\u044C \u043A\u043B\u0438\u0435\u043D\u0442\u0430"}<span class="clients__id">${this?._data ? "id:" : ""}${this._data?.id.slice(7) || " "}</span></h2>
        <input class="form__input" type="text" placeholder="\u{424}\u{430}\u{43C}\u{438}\u{43B}\u{438}\u{44F}" name="surname" value = "${this._data?.surname || ""}">
        <input class="form__input" type="text" placeholder="\u{418}\u{43C}\u{44F}" name="name" value = "${this._data?.name || ""}">
        <input class="form__input" type="text" placeholder="\u{41E}\u{442}\u{447}\u{435}\u{441}\u{442}\u{432}\u{43E}"  name="lastName" value = "${this._data?.lastName || ""}">
    </div>
      <div class="form__contacts ${this._data?.contacts ? "form__contacts--add" : ""} select ps">
        ${this._data?.contacts ? this._data.contacts.map((contact)=>this._generateMarkupContactBar(contact)).join("") : ""}  
             
      </div>
    <div class="form__contacts-btn ${this._data?.contacts ? "form__contacts-btn--expand" : ""}">
    <button type='button' class=" btn-reset btn-add-contacts">
    <span class="btn--add">\u{414}\u{43E}\u{431}\u{430}\u{432}\u{438}\u{442}\u{44C} \u{43A}\u{43E}\u{43D}\u{442}\u{430}\u{43A}\u{442}</span>
    </button>
    </div>
    
    <button  type='submit' class="btn-reset btn btn--save">\u{421}\u{43E}\u{445}\u{440}\u{430}\u{43D}\u{438}\u{442}\u{44C}</button>
    `;
    }
    _generateMarkupDelete(title) {
        return `
      <h2 class="modal__title title--delete text-reset">${title}</h2>
    <p class="modal__text" >\u{412}\u{44B} \u{434}\u{435}\u{439}\u{441}\u{442}\u{432}\u{438}\u{442}\u{435}\u{43B}\u{44C}\u{43D}\u{43E} \u{445}\u{43E}\u{442}\u{438}\u{442}\u{435} \u{443}\u{434}\u{430}\u{43B}\u{438}\u{442}\u{44C} \u{434}\u{430}\u{43D}\u{43D}\u{43E}\u{433}\u{43E} \u{43A}\u{43B}\u{438}\u{435}\u{43D}\u{442}\u{430}?</p>
   
  <button class="btn-reset btn btn--save">\u{423}\u{434}\u{430}\u{43B}\u{438}\u{442}\u{44C}</button>
  `;
    }
    ...
    <select  class="input__list" name="type">
      ...
      <option value="phone" ${contact.type === "phone" && "selected"}>\u{422}\u{435}\u{43B}\u{435}\u{444}\u{43E}\u{43D}</option>
      <option value="email" ${contact.type === "email" && "selected"}>Email</option>
      <option value="fb" ${contact.type === "fb" && "selected"}>Facebook</option>
      <option value="rest" ${contact.type === "rest" && "selected"}>\u{414}\u{43E}\u{43F}.\u{442}\u{435}\u{43B}\u{435}\u{444}\u{43E}\u{43D}</option>
    </select>
    <input
      class="input__value"
      type="text"
      placeholder="\u{412}\u{432}\u{435}\u{434}\u{438}\u{442}\u{435} \u{434}\u{430}\u{43D}\u{43D}\u{44B}\u{435} \u{43A}\u{43E}\u{43D}\u{442}\u{430}\u{43A}\u{442}\u{430}"
      name = "value" 
  ...
  `;
    ...
    addHandlerSubmit(handler) {
        this._parentEl.addEventListener("submit", function(e) {
            ...
            const client = Object.fromEntries(dataArr.filter((entry)=>!(entry[0].startsWith("type") || entry[0].startsWith("value"))));
            client.contacts = dataArr.filter((entry)=>entry[0].startsWith("type") || entry[0].startsWith("value")).map((contact, i, arr)=>{
                if (i % 2 === 0) return Object.fromEntries([
                    contact,
                    arr[i + 1]
                ]);
    ...
    _addOrDeleteContact(e) {
        ...
        const numOfContacts = document?.querySelectorAll(".select__input").length + 1;
        if (btnDeleteContact) {
    ...
    _addHandlerModalContact() {
        this._parentEl.addEventListener("click", this._addOrDeleteContact.bind(this));
    }
}
exports.default = new ModalView();

},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3","url:../../images/icons.svg":"4Uvst","./view":"7MbWE"}],"lozop":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
var _view = require("./view");
var _viewDefault = parcelHelpers.interopDefault(_view);
class SortView extends (0, _viewDefault.default) {
    ...
    addHandlerSorting(handler) {
        this._parentEl.addEventListener("click", async function(e) {
            ...
            if (sortByID) // взять тип сортировки из дата аттрибута
            handler("sortByID");
            if (sortByName) handler("sortByName");
            if (sortByCreate) handler("sortByCreate");
            if (sortByEdit) handler("sortByEdit");
        });
    }
}
exports.default = new SortView();

},{"./view":"7MbWE","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["7uCb0","1GgH0"], "1GgH0", "parcelRequire1ec5")
